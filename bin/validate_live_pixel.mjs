...
function queryClickhouse(pixelDefs) {
    ...
    for (const pixel of Object.keys(pixelDefs)) {
        ...
        const queryString = `SELECT DISTINCT request FROM metrics.pixels WHERE pixel_id = '${pixelID}' AND date > now() - INTERVAL 30 DAY AND pixel ILIKE '${pixel}%' AND request NOT ILIKE '%test=1%' ${agentString} LIMIT 10000`;
